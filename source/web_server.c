...
static SessionPrivate session_create(int fd, SSL_CTX *ctx)
{
...
static void session_destroy(SessionPrivate session)
{
...
static void accept_client(EventObject arg)
{
	...
	SessionPrivate session = session_create(clnt_fd, server->ctx);
	if (session == NULL)
		goto CLOSE_FD;
